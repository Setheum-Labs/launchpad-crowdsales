...
benchmark-pallet:
	cd s-node && cargo run --release --features=with-ethereum-compatibility -- benchmark --chain=dev --steps=50 --repeat=20 '--pallet=launchpad_crowdsales' '--extrinsic=*' --execution=wasm --wasm-execution=compiled --heap-pages=4096 --template=./templates/weight-module-template.hbs --output=./launchpad-crowdsales/src/weights.rs
