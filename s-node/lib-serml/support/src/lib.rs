...
pub trait Proposal<AccountId, BlockNumber> {
	type CampaignId: FullCodec + Debug + Clone + Eq + PartialEq;

	/// Get all proposals
	fn all_proposals() -> Vec<CampaignInfo<AccountId, AsBalance, BlockNumber>>;
	/// The Campaign Proposal info of `id`
	fn proposal_info(id: Self::CampaignId) -> Option<CampaignInfo<AccountId, AsBalance, BlockNumber>>;
    ...
    fn on_approve_proposal(id: Self::CampaignId) -> sp_std::result::Result<(), DispatchError>;
	/// Reject Proposal by `id` and update storage
	fn on_reject_proposal(id: Self::CampaignId) -> sp_std::result::Result<(), DispatchError>;
	/// Remove Proposal by `id` from storage
	fn remove_proposal(id: Self::CampaignId) -> sp_std::result::Result<(), DispatchError>;
}
...
pub trait CampaignManager<AccountId, BlockNumber> {
	...
	fn campaign_info(id: Self::CampaignId) -> Option<CampaignInfo<AccountId, AsBalance, BlockNumber>>;
	/// Get all proposals
	...
	fn activate_campaign(id: Self::CampaignId) -> DispatchResult;
	...
	fn on_successful_campaign(now: BlockNumber, id: Self::CampaignId) -> DispatchResult ;
	/// Record Failed Campaign by `id`
	fn on_failed_campaign(now: BlockNumber, id: Self::CampaignId) -> DispatchResult ;
