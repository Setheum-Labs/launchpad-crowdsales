...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case 'LOAD_PROPERTIES':
        return { ...state, propertiesState: 'LOADING' }
    case 'SET_PROPERTIES':
      return { ...state, properties: action.payload, propertiesState: 'READY' }
    default:
...
const loadAccounts = (state, dispatch) => {
  ...
  const asyncLoadAccounts = async () => {
    try {
      await web3Enable('setheum-launchpad-crowdsales')
...
const SubstrateContextProvider = props => {
  ...
  useEffect(() => {
    ...
    if (apiState === 'READY' && !keyringState && !keyringLoadAll) {
      ...
      getTokenProperties(state, dispatch)
    }
  }, [state, dispatch])
...
const getTokenProperties = async (state, dispatch) => {
  const { api } = state
  ...
  const symbols =
    !params.tokenSymbol.isNone && params.tokenSymbol.value.toHuman();
  ...
  dispatch({ type: 'SET_PROPERTIES', payload: symbolsDecimals })
  return {
    decimals,
    symbols,
    symbolsDecimals,
...
export { SubstrateContextProvider, useSubstrate, useSubstrateState, getTokenProperties }
